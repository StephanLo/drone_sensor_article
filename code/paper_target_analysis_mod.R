...
in_1 <- p_buf$no %in% 1:2 # this logical vector is true at the index where no is 1 or 2
in_2 <- p_buf$no %in% 3:5 # we can use it to index         
...
vi_points <- rbind(points_s1,
                   points_s2,
                   ortho_points)

vi_points$ID <- 1:nrow(vi_points)
